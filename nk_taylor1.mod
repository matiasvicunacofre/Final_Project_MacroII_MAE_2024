...
// Variables endógenas
var y pi i y_na y_gap y_hat r r_na  z a n ;
...
rho = (1/beta) - 1; //Tasa real natural
...
// Curva de Phillips
y_gap = y_gap(+1) - (1 / sigma) * (i - pi(+1) - r_na);

//Tasa de interés natural - de precios flexibles
r_na = rho + (1 - rho_z) * z - sigma * psi_ya * a * (1 - rho_a);
...
y_hat = y_na - steady_state(y); 
    

// Shocks lineales : de productividad y de preferencias
z = rho_z * z(-1) + epsilon_z; //Shock de preferencias
a = rho_a * a(-1) + epsilon_a; // Shock de productividad
...
@#define nonsimul  = 1   
@#define simultech = 0   //Shock tecnológico
@#define simuldem = 0   //Shock de preferencias
@#define simulboth = 0 //Shocks simultáneos


@#if simultech ==1
// Shock tecnológico aislado
...
y_t = strmatch('y_gap', M_.endo_names, 'exact');
...
//Calcular de valores de L (pérdida de bienestar promedio)
...
//Código fuente para la replicación de la tabla 4.1 
@#if nonsimul == 1
...
for j = 1:length(phis_pi)
    ...
    results_demand(2, j) = oo_.var(3,3)^0.5; // Varianza de y_gap
